...
/// A modern, professional admin reports dialog showing supervisor-wise report statistics
class AdminReportsDialog extends StatefulWidget {
  final dynamic admin;
  final List<Map<String, dynamic>> adminSupervisorsWithStats;

  const AdminReportsDialog({
    super.key,
    required this.admin,
  ...
  @override
  State<AdminReportsDialog> createState() => _AdminReportsDialogState();
}

class _AdminReportsDialogState extends State<AdminReportsDialog>
    with TickerProviderStateMixin {
  String searchQuery = '';
  String sortBy = 'name'; // name, reports, completion
  ...
  Widget _buildOverallStats(BuildContext context, bool isDark) {
    final totalSupervisors = widget.adminSupervisorsWithStats.length;
    int totalReports = 0;
    int completedReports = 0;
    int totalMaintenance = 0;
    int completedMaintenance = 0;

    for (final supervisor in widget.adminSupervisorsWithStats) {
          ...
          child: DropdownButtonHideUnderline(
            child: DropdownButton<String>(
              ...
              items: const [
                ...
                DropdownMenuItem(value: 'reports', child: Text('البلاغات')),
    ...
    filteredSupervisors.sort((a, b) {
      ...
      switch (sortBy) {
        case 'name':
          ...
          break;
        case 'reports':
          final aReports = (aStats['reports'] as int? ?? 0);
          final bReports = (bStats['reports'] as int? ?? 0);
          comparison = aReports.compareTo(bReports);
          break;
        case 'completion':
          final aTotal = (aStats['reports'] as int? ?? 0) +
              (aStats['maintenance'] as int? ?? 0);
          final aCompleted = (aStats['completed_reports'] as int? ?? 0) +
              (aStats['completed_maintenance'] as int? ?? 0);
          final aRate = aTotal > 0 ? (aCompleted / aTotal) : 0.0;

          final bTotal = (bStats['reports'] as int? ?? 0) +
              (bStats['maintenance'] as int? ?? 0);
          final bCompleted = (bStats['completed_reports'] as int? ?? 0) +
              (bStats['completed_maintenance'] as int? ?? 0);
                ...
                Expanded(
                  child: Column(
                    ...
                    children: [
                      ...
                      Row(
                        children: [
                          _buildMiniStat('البلاغات', totalReports.toString(),
                              const Color(0xFF3B82F6)),
                          const SizedBox(width: 16),
                  ...
                  children: [
                    ...
                    Wrap(
                      ...
                      children: [
                        ...
                        // Late Completed Reports
                        if (lateCompletedReports > 0)
                          _buildStatusDot(
                            'بلاغات متأخرة مكتملة',
    ...
    return Center(
      child: Column(
        ...
        children: [
          ...
          const SizedBox(height: 8),
          Text(
            'قم بتعيين مشرفين لهذا المسؤول لعرض بلاغات البلاغات',
            style: TextStyle(
              fontSize: 14,
              color: isDark ? const Color(0xFF94A3B8) : const Color(0xFF6B7280),
            ),
